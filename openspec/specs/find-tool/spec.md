...
The `find` tool SHALL accept a `pattern` string parameter (required) and a `path` string parameter (optional). The tool SHALL recursively search the directory tree rooted at `path` (or the session's current working directory if `path` is omitted) and return all file and directory paths whose relative path matches `pattern` using doublestar glob syntax.
...
- **WHEN** `find` is called with `pattern: "*.go"` in a directory containing `main.go`, `internal/tools/grep.go`, and `README.md`
- **THEN** the result contains `main.go` and `internal/tools/grep.go` but not `README.md`

#### Scenario: Recursive doublestar pattern
- **WHEN** `find` is called with `pattern: "**/*.test.ts"` in a directory containing `src/app.test.ts` and `src/utils/helper.test.ts`
- **THEN** the result contains both files

#### Scenario: Directory-scoped pattern
- **WHEN** `find` is called with `pattern: "*.go"` and `path: "internal/tools"`
...
- **WHEN** `find` is called with `pattern: "src/**/*.md"` in a directory containing `src/README.md` and `docs/README.md`
- **THEN** the result contains `src/README.md` but not `docs/README.md`

#### Scenario: Brace expansion
- **WHEN** `find` is called with `pattern: "*.{ts,tsx}"`
- **THEN** the result contains both `.ts` and `.tsx` files

#### Scenario: Character class
- **WHEN** `find` is called with `pattern: "[Mm]akefile"`
- **THEN** the result contains files named `Makefile` or `makefile`

### Requirement: Find returns results sorted by modification time
...
- **WHEN** `find` is called and file `a.go` was modified 1 hour ago and `b.go` was modified 1 minute ago
- **THEN** `b.go` appears before `a.go` in the output

### Requirement: Find returns relative paths
...
- **WHEN** `find` is called with `pattern: "*.go"` and `path: "/workspace/project"`
- **THEN** the result contains paths like `main.go` and `internal/tools/grep.go`, not `/workspace/project/main.go`

### Requirement: Find returns "No files found" for zero results
...
#### Scenario: No matches
- **WHEN** `find` is called with `pattern: "*.xyz"` and no files match
- **THEN** the result text is "No files found" and `IsError` is false

#### Scenario: Non-existent path
- **WHEN** `find` is called with `path` pointing to a directory that does not exist
- **THEN** the result text is "No files found" and `IsError` is false

### Requirement: Find truncates output at 30,000 characters
...
- **WHEN** `find` is called with a broad pattern that matches thousands of files
- **THEN** the output is truncated at 30,000 characters with a truncation message appended

### Requirement: Find does not follow symbolic links to directories
...
- **WHEN** a directory contains a symlink `linked_dir -> /some/other/directory` and `find` is called with `pattern: "**/*.go"`
- **THEN** files inside the symlink target are NOT included in results

#### Scenario: File symlink included
- **WHEN** a directory contains a symlink `link.go -> ../other/file.go` and `find` is called with `pattern: "*.go"`
...
### Requirement: Find skips .git and respects .gitignore
...
- **WHEN** `find` is called with `pattern: "**/*"` in a git repository
- **THEN** files inside `.git/` are not included in results

#### Scenario: node_modules skipped
- **WHEN** `find` is called with `pattern: "**/*.js"` in a project with `node_modules/`
- **THEN** files inside `node_modules/` are not included in results

#### Scenario: Gitignored files excluded
- **WHEN** a `.gitignore` contains `*.log` and `find` is called with `pattern: "**/*.log"`
...
### Requirement: Find includes hidden files
...
- **WHEN** `find` is called with `pattern: "**/*"` in a directory containing `.github/workflows/ci.yml` and `.dockerignore`
- **THEN** both `.github/workflows/ci.yml` and `.dockerignore` appear in results

### Requirement: Find resolves paths and checks scoping
...
### Requirement: Find type filter (default mode only)
In default mode (not `--anthropic-compat`), the `find` tool SHALL accept an optional `type` string parameter with values `"file"` or `"directory"`. When set to `"file"`, only regular files (and file symlinks) SHALL be returned. When set to `"directory"`, only directories SHALL be returned. When omitted, both files and directories are returned.

#### Scenario: Type filter file
- **WHEN** `find` is called with `pattern: "**/*"` and `type: "file"` in a directory containing files and subdirectories
- **THEN** only files are returned, no directories

#### Scenario: Type filter directory
- **WHEN** `find` is called with `pattern: "**/*"` and `type: "directory"`
- **THEN** only directories are returned, no files

#### Scenario: Invalid type value
- **WHEN** `find` is called with `type: "symlink"`
- **THEN** the tool returns `IsError: true` indicating valid type values are "file" and "directory"

### Requirement: Find in anthropic-compat mode uses Glob schema
When `--anthropic-compat` is set, the tool SHALL be named `Glob` (not `find`). The schema SHALL have exactly two parameters: `pattern` (required string) and `path` (optional string). The `type` parameter SHALL NOT be present in the compat schema. The tool description SHALL match Claude Code's exact description text.

#### Scenario: Compat mode tool name
- **WHEN** boris is started with `--anthropic-compat`
- **THEN** the tool list includes `Glob` and does not include `find`
...
#### Scenario: Default mode tool name
- **WHEN** boris is started without `--anthropic-compat`
- **THEN** the tool list includes `find` and does not include `Glob`
