...
The `grep` tool SHALL accept an optional `include` string parameter containing a glob pattern (e.g., `"*.py"`, `"*.{ts,tsx}"`, `"src/**/*.py"`). When set, only files matching the glob SHALL be searched. The glob SHALL be matched first against the file's relative path from the search root (supporting path-qualified globs like `"src/**/*.py"`), then against the file's base name as a fallback (supporting simple extension globs like `"*.py"`). Matching uses `doublestar.Match` semantics (supporting brace expansion like `{ts,tsx}`).
...
#### Scenario: Include filter with path glob
- **WHEN** `grep` is called with `include: "src/**/*.py"` on a directory containing `src/utils/helper.py` and `tests/test.py`
- **THEN** only `src/utils/helper.py` is searched
...
The `grep` tool SHALL accept optional integer parameters `head_limit` (default 0, meaning unlimited) and `offset` (default 0). `head_limit` caps the number of output lines/entries returned. `offset` skips the first N output lines/entries before applying `head_limit`.

In `content` mode, `head_limit` and `offset` count **all output lines** uniformly — match lines, context lines, and `--` separator lines all count equally toward the limit. In `files_with_matches` mode, they cap/skip file paths (applied after mtime sorting). In `count` mode, they cap/skip file entries.

#### Scenario: head_limit truncates results
- **WHEN** `grep` is called with `head_limit: 10` and there are 50 output lines
- **THEN** the result contains the first 10 output lines only

#### Scenario: Unlimited by default
- **WHEN** `grep` is called without `head_limit` (or `head_limit: 0`) and there are 200 output lines
- **THEN** all 200 output lines are returned

#### Scenario: Offset skips results
- **WHEN** `grep` is called with `head_limit: 10, offset: 20` and there are 50 output lines
- **THEN** the result contains output lines 21-30 (skip first 20, then take 10)

#### Scenario: Offset without head_limit
- **WHEN** `grep` is called with `offset: 10` and there are 50 output lines
- **THEN** the result contains output lines 11-50 (skip first 10, return the rest)

#### Scenario: Offset exceeds total results
- **WHEN** `grep` is called with `offset: 100` and there are 50 output lines
- **THEN** the result is empty (no error)

#### Scenario: head_limit with context counts all lines
- **WHEN** `grep` is called with `head_limit: 2` and `context: 1`, producing a match with 1 context line before and 1 after
- **THEN** only the first 2 output lines are returned (context and match lines count equally)

#### Scenario: files_with_matches offset applied after mtime sort
- **WHEN** `grep` is called with `output_mode: "files_with_matches"` and `offset: 1` on 3 files with different mtimes
- **THEN** the newest file is skipped (offset applied after mtime sorting, not during walk)

### Requirement: Skip binary files
The `grep` tool SHALL skip binary files during search. Binary detection SHALL read the first 512 bytes of each file and scan for NUL (`\x00`) bytes. Any file whose header contains a NUL byte SHALL be treated as binary and skipped silently. This matches ripgrep's binary detection approach.

#### Scenario: Binary file skipped
- **WHEN** the search directory contains a file with NUL bytes in its header
- **THEN** the binary file is not searched and does not appear in results

#### Scenario: Text file searched
- **WHEN** the search directory contains a `.py` file with no NUL bytes
- **THEN** the file is searched normally

### Requirement: Skip noise directories
The `grep` tool SHALL skip `.git/` and `node_modules/` directories during recursive directory traversal. These directories SHALL be skipped during the walk to avoid descending into them entirely.
...
For `files_with_matches` and `count` modes, the match is checked against the full file content. For `content` mode, each match's byte range SHALL be mapped back to line numbers, and all lines spanned by the match SHALL be reported as match lines. In `count` mode, the count SHALL reflect the number of matching lines (not the number of regex matches). Context lines work based on the line range of the match.
...
#### Scenario: Multiline count reports matching lines
- **WHEN** a pattern matches 3 times across 2 lines and `output_mode: "count"` with `multiline: true`
- **THEN** the count is 2 (number of matching lines, not regex matches)
